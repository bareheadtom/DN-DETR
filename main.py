...
# norm_type='FrozenBatchNorm2d', batch_size=1, bbox_loss_coef=5, box_noise_scale=0.4, clip_max_norm=0.1, cls_loss_coef=1, co
# co_panoptic_path=None, coco_path='', contrastive=False, dataset_file='exdark', debug=False, dec_layers=6, dec_n_points=4, 
# device='cuda', dice_loss_coef=1, dilation=False, dim_feedforward=2048, dist_url='env://', distributed=False, drop_lr_now=F
# alse, dropout=0.0, enc_layers=6, enc_n_points=4, eos_coef=0.1, epochs=100, eval=False, find_unused_params=False, finetune_
# ignore=['label_enc.weight', 'class_embed'], fix_size=False, focal_alpha=0.25, frozen_weights=None, giou_loss_coef=2, hidde
# n_dim=256, label_noise_scale=0.2, local_rank=0, lr=0.0001, lr_backbone=1e-05, lr_drop=40, mask_loss_coef=1, masks=False, m
# odelname='dn_dab_detr', nheads=8, note='', num_feature_levels=4, num_patterns=0, num_queries=300, num_results=300, num_sel
# ect=300, num_workers=10, output_dir='/root/autodl-tmp/outputs/DNDETR/20240102105624', override_resumed_lr_drop=False, pe_t
# emperatureH=20, pe_temperatureW=20, position_embedding='sine', pre_norm=False, pretrain_model_path='./pretrained/dncheckpo
# int.pth', random_refpoints_xy=False, rank=0, remove_difficult=False, resume='', return_interm_layers=False, save_checkpoin
...
# 0001, world_size=1)
outputPath = '../../outputs/DNDETR/'+str(datetime.datetime.now().strftime("%Y%m%d%H%M%S")) + "dn_dab_detr_withdnonly_featenhanceraddatten"
...
class summaryWriterWrap:
    def __init__(self,summaryWriter) -> None:
        self.writer=summaryWriter
        self.itera = 0
summaryWriter = summaryWriterWrap(summaryWriter)
...
def get_args_parser():
    ...
    # * Backbone
    # parser.add_argument('--backbone', default='swin_L_224_22k', type=str,
    ...
    # Traing utils
    parser.add_argument('--output_dir', default=outputPath, help='path where to save, empty for no saving')
...
def main(args):
    ...
    if not args.resume and args.pretrain_model_path:
        print("user pretrained weight ",args.pretrain_model_path)
    ...
    #         wo_class_error=wo_class_error, args=args, logger=(logger if args.save_log else None),summaryWriter = summaryWriter
    #     )
    # for k,v in test_stats.items():
    #     summaryWriter.writer.add_scalar(k,v,1000)
    #     break
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            args.clip_max_norm, wo_class_error=wo_class_error, lr_scheduler=lr_scheduler, args=args, logger=(logger if args.save_log else None),summaryWriter = summaryWriter)
        if args.output_dir:
        ...
        for k,v in test_stats.items():
            summaryWriter.writer.add_scalar(k,v,epoch)
            break
